...
const validateSignup = (req, res, next) => {
    const schema = Joi.object({
        name: Joi.string().min(2).max(225).required().messages({
            ...
            'string.max': 'The name must not be more than 225 characters (e.g., John, Betty)',
            'string.empty': 'Name cannot be empty (e.g., John, Betty)',
            'any.required': 'A name is required (e.g., John, Betty)'
        }),
        email: Joi.string().email().min(5).max(225).required().messages({
